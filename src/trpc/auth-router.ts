...
export const authRouter = router({
    createPayloadUser: publicProcdeure.input(AuthCredentialsValidator).mutation(async ({input}) => {
    ...
    signIn: publicProcdeure.input(AuthCredentialsValidator).mutation(async ({input,ctx}) => {
        const{email, password} = input
        const { res } = ctx
        const payload = await getPayloadClient()
